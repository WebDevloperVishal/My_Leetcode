...
import { db } from '../lib/db.js';
// Import Judge0 helper functions for code execution
import {
  getLanguageName,
  pollBatchResults,
  submitBatch,
...
export const executeCode = async (req, res) => {
  ...
  try {
    ...
    // 📁 8. Save individual test case results using detailedResults directly
    // Prepare test case results array for bulk insertion
    const testCaseResults = detailedResults.map((result) => ({
      submissionId: submission.id,
      testCase: result.testCase,
      passed: result.passed,
    ...
    // Insert all test case results into database in one operation
    await db.testCaseResult.createMany({ data: testCaseResults });

    // 🔍 9. Fetch full submission with test cases
    // Get the complete submission details including all test case results
    const submissionWithTestCases = await db.submission.findUnique({
      where: { id: submission.id },
      include: { testCases: true },
    });
    ...
    // 📤 10. Respond to client
    // Send success response with complete submission details
    res.status(200).json({
      success: true,
      message: 'Code executed successfully',
      submission: submissionWithTestCases,
